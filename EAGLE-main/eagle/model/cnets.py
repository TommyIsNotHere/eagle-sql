...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            rope_scaling = dict(self.config.rope_scaling or {})
            # Compat across transformers versions:
            # - old key: type
            # - new key: rope_type
            scaling_type = rope_scaling.get("type", rope_scaling.get("rope_type", "default"))
            scaling_factor = rope_scaling.get("factor", 1.0)
            rope_scaling.setdefault("rope_type", scaling_type)
            rope_scaling.setdefault("type", scaling_type)
            rope_scaling.setdefault("factor", scaling_factor)
            ...
            if scaling_type == "linear":
                ...
                # Treat default as non-scaled RoPE for backward compatibility.
                if hasattr(self.config, "rope_theta"):
